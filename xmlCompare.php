<?php
// Load the PhpSpreadsheet library (install via Composer if needed)
...
// Set column headers
$sheet->setCellValue('A1', 'Production URL');
$sheet->setCellValue('B1', 'XML URL');

// Determine the maximum number of rows needed
$maxRows = max(count($productionUrls), count($xmlUrls));

// Write URLs to the spreadsheet
for ($i = 0; $i < $maxRows; $i++) {
    // Get URLs for the current row (or empty string if no more URLs)
    $prodUrl = isset($productionUrls[$i]) ? $productionUrls[$i] : '';
    $xmlUrl = isset($xmlUrls[$i]) ? $xmlUrls[$i] : '';
    
    // Write to columns A and B
    $sheet->setCellValue('A' . ($i + 2), $prodUrl);
    $sheet->setCellValue('B' . ($i + 2), $xmlUrl);
}

// Save the spreadsheet as an Excel file
$writer = new Xlsx($spreadsheet);
$outputFile = 'url_comparison_sorted.xlsx';
$writer->save($outputFile);

echo "Excel file created successfully: $outputFile\n";
?>